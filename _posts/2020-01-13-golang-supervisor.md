---
layout: post
title: 使用supervisor守护golang服务
date: 2020-01-13
categories: blog
tags: [golang,go,supervisor]
description: 使用supervisor守护golang服务

---

## 问题与思考

在golang项目部署中，最简单的方式是（例如我的项目叫demo）：

```bash
go build -o demo main.go
nohup ./demo > demo.out 2>&1 &
```

有一个问题就是，因为有时候会用到一些第三方插件，例如，缓存组件，数据库组件，socket服务等等，
有时候你并没有留意第三方组件有`panic`，并且调用了没有`recover`，就会导致整个项目停止运行。

## 1、安装supervisor

由于我使用的ubuntu18.4，所以直接用apt-get安装
...
```bash
[program:demo];程序名称，终端控制时需要的标识
user=hiho; 进程执行的用户身份
command=nohup ./demo > demo.out 2>&1 &; 运行程序的命令; 运行程序的命令
directory=/hiho/home/www/demo/ ;命令执行的目录
autostart=true;表示是否跟随supervisor一起启动。
autorestart=true;如果该程序挂了，是否重新启动。
startsecs=10
stdout_logfile=/home/hiho/www/golang/xjr/demo/demo.out.log; 输出日志文件
stdout_logfile_maxbytes=1MB
stdout_logfile_backups=10
stdout_capture_maxbytes=1MB
stderr_logfile=/home/hiho/www/golang/xjr/demo/demo.err.log; 错误日志文件
...
编译go脚本：  
由于在supervisor中部署编译，可能会不能识别go的依赖的环境配置，所以直接在外面编译后，supervisor直接运行可执行文件
...
![1.png](/source/images/golang-supervisor/1.png)

## 启动失败排查

如果启动失败，可以通过配置的`stderr_logfile`或者`/var/log/supervisor/supervisord.log`查看错误信息进行排查

## 常用的相关管理命令

```bash
sudo supervisorctl restart <application name> ;重启指定应用
sudo supervisorctl stop <application name> ;停止指定应用
sudo supervisorctl start <application name> ;启动指定应用
sudo supervisorctl staus <application name>  ;查看应用状态
sudo supervisorctl restart all ;重启所有应用
sudo supervisorctl stop all ;停止所有应用
sudo supervisorctl start all ;启动所有应用
```
